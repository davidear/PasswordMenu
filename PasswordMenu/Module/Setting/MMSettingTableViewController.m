...
- (void)loadData {
    NSString *path =
    [[NSBundle mainBundle] pathForResource:@"SettingList" ofType:@"plist"];
    self.dataArray = [NSMutableArray arrayWithContentsOfFile:path];
}
- (void)setUI {
    [self.tableView registerClass:[MMSettingSwitchCell class]
           forCellReuseIdentifier:SwitchReuseIdentifier];
    UILabel *header = [[UILabel alloc]
                       initWithFrame:CGRectMake(0, 0, self.tableView.bounds.size.width, 21)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row < 2) {
            ...
            switchButton.enabled = [[VENTouchLock sharedInstance] isPasscodeSet] &&
            [VENTouchLock canUseTouchID];
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            if ([[VENTouchLock sharedInstance] isPasscodeSet]) { //已设置密码
                VENTouchLockEnterPasscodeViewController *vc =
                [[VENTouchLockEnterPasscodeViewController alloc] init];
                __weak typeof(vc) weakself = vc;
                vc.willFinishWithResult = ^void(BOOL success) {
                    if (success) {
                        VENTouchLockCreatePasscodeViewController *createVC =
                        [[VENTouchLockCreatePasscodeViewController alloc] init];
                        [weakself.navigationController pushViewController:createVC
                                                                 animated:YES];
                    }
                };
                MMNavigationController *nvc =
                [[MMNavigationController alloc] initWithRootViewController:vc];
                [self presentViewController:nvc animated:YES completion:nil];
                
            } else { //未设置密码
                VENTouchLockCreatePasscodeViewController *vc =
                [[VENTouchLockCreatePasscodeViewController alloc] init];
                MMNavigationController *nvc =
                [[MMNavigationController alloc] initWithRootViewController:vc];
                [self presentViewController:nvc animated:YES completion:nil];
